...
pb () {
	...
	if [ -e $ROM ] ; then
		...
		cp -p ../tools/script/21pricachemount system/etc/init.d/
		
		$BIN/zip -ru /sdcard/c2dsd/Primary-Mod/RDBSD_Pri_${DATE}_$FILE *
...
ab () {
	...
	if [ -e $ROM ] ; then
		...
		cp -p ../tools/script/21altcachemount system/etc/init.d/
		
		$BIN/zip -ru /sdcard/c2dsd/Alternate-Mod/RDBSD_Alt_${DATE}_$FILE *
